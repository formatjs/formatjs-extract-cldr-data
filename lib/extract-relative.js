...
function loadRelativeFieldsData(locales) {
    return locales.reduce(function (data, locale) {
        var filename = glob.sync(path.join(CLDR_PATH, 'main', locale, 'dateFields.json'), { nocase: true })[0];
        var main     = require(filename).main;

        Object.keys(main).forEach(function (key) {
            main[key].localeName = key;
            main[key.toLowerCase()] = main[key];
        });

        var localeData = require(filename).main[locale.toLowerCase()];
        var fields = localeData.dates.fields;

        // Reduce the date fields data down to whitelist of fields needed in the
        // FormatJS libs.
        data[localeData.localeName] = FIELD_NAMES.reduce(function (relative, field) {
